...
//对某地址写入8bit数据
uint8_t write_to_address(char* address_str, char* bin)
{
    printf("write to address:%s,%s\n",address_str,bin);
    
    //将pin_1和pin_2对应bit位一致
    //判断当前短路pin类型，将两个pin对应bit为一致
    int index1,index2=0;
